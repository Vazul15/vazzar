...
export const StreamingProvidersPage = () => {
  ...
  const [moviesOfproviders, setMoviesOfProviders] = useState<topMoviesGeneral>({
    page: 1,
    movies: [],
  });
  ...
  const handleSelection = (selectedProvider: providerType): void => {
    setSelectedProvider((provider) =>
      provider && provider.providerId === selectedProvider.providerId ? null : selectedProvider
    );
  ...
  return (
    <div>
      ...
      {isSelected && selectedProvider && (
        <div>
          <GalleryForAllMovies movies={moviesOfproviders} title={`Movies > ${selectedProvider.providerName}`} />
        </div>
      )}
    </div>
  );
