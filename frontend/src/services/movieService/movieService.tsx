import { movieDetailsAndCredits, topMoviesGeneral } from "@/components/types/movieTypes";
import {
  genresType,
  providerType,
...
export async function getDetailsAndCreditsOfMovieByMovieId(movieId: number): Promise<movieDetailsAndCredits> {
  const response = await fetch(`/api/movies/movie-details?movieId=${movieId}`);
