...
import { getGenres, getMoviesByGenre } from "@/services/movieService/movieService";
...
export const MoviesByGenrePage = () => {
  ...
  const getnameById = (genreId: number) => {
    const genreFound = genresNameId.find((genreMap) => genreMap.id === genreId);
    if (genreFound) {
  ...
  useEffect(() => {
    const fetchMoviesByGenreAndGenreName = async () => {
      if (genreId) {
        const movieData = await getMoviesByGenre(Number(genreId), watchRegion);
  ...
  return (
    <div>
      <GalleryForAllMovies movies={moviesByGenre} title={`Movies > ${genreName}`}/>
    </div>
  );
};
